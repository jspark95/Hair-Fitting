function [ im ] = visualize_result( vert3D, vert2D, face, width, height, background, color )

UV = vert2D;
Z = -vert3D(:, 3);
Nvertices   = size(vert3D, 1);
...
f       = find(any(ismember(face, v), 2));
Nfaces  = length(f);
 
% Compute the edge vectors
e1s = vert3D(v2(f), :) - vert3D(v1(f), :);
e2s = vert3D(v3(f), :) - vert3D(v1(f), :);
e3s = vert3D(v2(f), :) - vert3D(v3(f), :);
...
view = vert3D ./ repmat(sqrt(sum(vert3D.^2,2)), 1, 3);
