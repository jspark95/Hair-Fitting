function [ im ] = visualize_result2( FV, R, s, t, width, height, background )
...
UV(:, 1)    = s.*(V2(:, 1)+t(1)) ;	% orthographic projection for x
UV(:, 2)    = s.*(V2(:, 2)+t(2)) ;	% orthographic projection for y

% Transform to the pixel plane (the axes remain switched)
UV(:, 1)    = width - UV(:, 1);
UV          = UV + 0.5;

% SW. Set UV map to be in the view frustum( regardless of z-coords.)
% UV(:, 1)    = UV(:, 1)-min(UV(:, 1));
% UV(:, 2)    = UV(:, 2)-min(UV(:, 2));
% UV          = UV + 1;
% UV(:,1) = (UV(:,1)./max(UV(:,1))).*width;
% UV(:,2) = (UV(:,2)./max(UV(:,2))).*height;
...
view = FV.vertices ./ repmat(sqrt(sum(FV.vertices.^2,2)), 1, 3);
specular = max(0, sum(view .* (2 * repmat(normals * [0; 0; 1],1,3).*normals - repmat([0; 0; 1]', length(normals), 1)),2));
diffuse = diffuse + specular;
% specular = FV.vertices * (2 * max(0, normals' * (normals * [0; 0; 1])) - [0; 0; 1])
...
% test2 = repmat(test,[1 1 3]);

im(~test)=background(~test);
im = 0.9*im + 0.3*background;
im(~test) = background(~test);
